---
title: "Korea Looks Cash-Optional — Until It Isn't: A Currency & Card Guide"
date: 2026-08-25
category: Money-saving
description: The card payment mistake that quietly costs you extra on every purchase, why Korea is trying to phase out coins, and the one situation where "cashless Korea" isn't actually true.
image_query: korean won cash market
image: https://raw.githubusercontent.com/solusupport-bot/land-in-korea-blog/media-assets/images/korea-currency-card-payment-mistake-1788111240471.jpg
---

Korean cities are largely cashless — most cafes, convenience stores, and restaurants take card, including foreign Visa/Mastercard. That's true right up until you're standing at a traditional market stall or a small rural spot that's cash-only, and by then it's too late to have planned for it.

## The mistake that costs you on every single card swipe

When a card terminal in Korea asks whether to charge you in your home currency or Korean won, choosing your home currency feels safer — it isn't. That's Dynamic Currency Conversion (DCC), and it adds a hidden markup on top of your own bank's exchange rate every time you accept it. Always choose to be charged in **Korean won (KRW)**, even if the terminal defaults to your home currency. Separately from DCC, check whether your own card or bank charges a foreign transaction fee on top of that — some travel-oriented cards and accounts waive this fee, which matters more the longer your trip runs.

## Where cash still matters

Small traditional markets, some rural areas, and a handful of older independent restaurants remain cash-preferred or cash-only. Carrying a modest cash buffer — enough for a market meal or a small shop — avoids getting stuck. One thing worth knowing if you do carry cash: Korea has been actively pushing a "coinless society" initiative for years, encouraging stores to let customers round small change into a [T-money card](https://landinkorea.com/posts/tmoney-first-timer-mistake.html) balance or a donation instead of handing back physical coins — so don't be surprised if a cashier offers that instead of jingling change, and don't assume every till has small coins on hand to break a large bill.

## Getting cash without getting stung

Look for ATMs marked "Global" or with a foreign card logo — Woori, KEB Hana, and 7-Eleven ATMs are generally the most reliable for foreign card acceptance. Convenience store ATMs are also usually available 24 hours, while some bank-branch ATMs restrict foreign card withdrawals to daytime hours for security reasons — worth remembering if you're getting cash late at night. Notify your bank of your travel dates before you leave, since an unannounced overseas card attempt is a common reason for a first-use freeze.

![atm machine convenience store](https://images.pexels.com/photos/32133697/pexels-photo-32133697.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=650&w=940)

## Better exchange rates, if you're converting cash

Currency exchange counters near [Myeongdong](https://landinkorea.com/posts/korea-myeongdong-shopping-district.html) and Hongdae typically beat airport kiosk rates by a noticeable margin. Exchange only what covers your first day or two at the airport, then top up in the city once you've found a better rate. Kiosk-based prepaid traveler card machines (WOWPASS is one well-known example found in areas like Myeongdong and some transit hubs) are another option — they let you exchange cash and load it onto a card usable like a debit card around the city, which can be convenient if you'd rather not carry a large amount of loose cash.

![currency exchange counter myeongdong](https://images.pexels.com/photos/13679285/pexels-photo-13679285.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=650&w=940)

## Mobile pay isn't the shortcut it looks like — for tourists, anyway

Samsung Pay and Naver Pay are widely used locally, but usually require a Korean bank account to set up — not something you can activate as a short-term visitor. This is different if you're a foreign resident with an Alien Registration Card rather than a tourist: opening a Korean bank account (and then setting up local mobile pay) is realistically possible once you have resident status, even though it isn't for a short tourist trip. For most short trips, a normal card charged in won stays the simplest option throughout — and it's worth knowing that contactless tap-to-pay support on Korean card terminals has historically been less universal than in Europe or North America, so having a physical chip card as a backup to a phone-based tap payment is a reasonable precaution.

## The short version

- Always choose to be charged in Korean won, never your home currency, at the terminal
- Check separately whether your own card charges a foreign transaction fee
- Keep some cash for traditional markets and small rural or older shops
- Coins are being phased down — don't expect every till to make exact change in coins
- Convenience store ATMs tend to run 24 hours; some bank ATMs restrict foreign cards to daytime
- Samsung Pay/Naver Pay generally need a Korean bank account — realistic for residents, not short-term tourists

## Frequently Asked Questions

### Should I choose to be charged in my home currency or Korean won?

Always choose Korean won (KRW) — accepting the home-currency option (Dynamic Currency Conversion) adds a hidden markup on top of your bank's exchange rate.

### Is Korea fully cashless?

No — traditional markets and some rural or older independent restaurants remain cash-preferred or cash-only, so carrying a modest cash buffer helps.

### Why did a cashier offer to round my change instead of giving coins?

Korea has been encouraging a "coinless society" for years, letting stores round small change into a T-money balance or donation instead of handing back physical coins — it's a normal, legitimate practice, not a scam.

### Are ATMs open 24 hours in Korea?

Convenience store ATMs generally are. Some bank-branch ATMs restrict foreign card withdrawals to daytime hours for security, so don't count on every ATM being available late at night.

### Can I use Samsung Pay or Naver Pay as a short-term visitor?

Usually not — both typically require a Korean bank account to set up, which realistically means resident status (an Alien Registration Card), not a short tourist trip. A normal card charged in won is the simplest option for most visits.

### Is tap-to-pay with my phone reliable in Korea?

Less universally than in some other countries — contactless terminal support has historically been inconsistent, so it's worth carrying a physical chip card as a backup rather than relying entirely on your phone.
